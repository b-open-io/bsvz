...
test "go direct script rows: exact depth and stack-index false results" {
    ...
    try runRows(allocator, flags, &[_]GoRow{
        ...
        .{ .name = "go row 687: double nop still leaves bare depth false", .unlocking_hex = "61", .locking_hex = "6174", .expected = .{ .success = false } },
        .{ .name = "go row 688: depth in unlocking script alone leaves false at the seam", .unlocking_hex = "74", .locking_hex = "", .expected = .{ .success = false } },
