...
test "go direct script rows: boolean and minmaxwithin parity" {
    ...
    try runRows(allocator, bsvz.script.engine.ExecutionFlags.legacyReference(), &[_]GoRow{
        ...
        .{ .row = 637, .name = "lessthanorequal with false and non-minimal false shape still drops to true tail", .unlocking_hex = "00020000", .locking_hex = "a17551", .expected = .{ .success = true } },
...
test "go direct script rows: comparison and minmax exact rows" {
    ...
    try runRows(allocator, flags, &[_]GoRow{
        ...
        .{ .row = 248, .name = "greaterthan over descending values stays true", .unlocking_hex = "5b5a", .locking_hex = gt_lock, .expected = .{ .success = true } },
        .{ .row = 249, .name = "greaterthan not over equal values becomes true", .unlocking_hex = "5454", .locking_hex = gt_not_lock, .expected = .{ .success = true } },
        .{ .row = 250, .name = "greaterthan not over ascending values becomes true", .unlocking_hex = "5a5b", .locking_hex = gt_not_lock, .expected = .{ .success = true } },
        .{ .row = 251, .name = "greaterthan not over negative and positive becomes true", .unlocking_hex = neg11_11, .locking_hex = gt_not_lock, .expected = .{ .success = true } },
        .{ .row = 252, .name = "greaterthan not over negative ascending becomes true", .unlocking_hex = neg11_neg10, .locking_hex = gt_not_lock, .expected = .{ .success = true } },
        .{ .row = 253, .name = "lessthanorequal not over descending becomes true", .unlocking_hex = "5b5a", .locking_hex = lte_not_lock, .expected = .{ .success = true } },
        .{ .row = 254, .name = "lessthanorequal over equal values stays true", .unlocking_hex = "5454", .locking_hex = lte_lock, .expected = .{ .success = true } },
        .{ .row = 255, .name = "lessthanorequal over ascending values stays true", .unlocking_hex = "5a5b", .locking_hex = lte_lock, .expected = .{ .success = true } },
        .{ .row = 256, .name = "lessthanorequal over negative and positive stays true", .unlocking_hex = neg11_11, .locking_hex = lte_lock, .expected = .{ .success = true } },
        .{ .row = 257, .name = "lessthanorequal over negative ascending stays true", .unlocking_hex = neg11_neg10, .locking_hex = lte_lock, .expected = .{ .success = true } },
        .{ .row = 258, .name = "greaterthanorequal over descending stays true", .unlocking_hex = "5b5a", .locking_hex = gte_lock, .expected = .{ .success = true } },
        .{ .row = 259, .name = "greaterthanorequal over equal values stays true", .unlocking_hex = "5454", .locking_hex = gte_lock, .expected = .{ .success = true } },
        .{ .row = 260, .name = "greaterthanorequal not over ascending becomes true", .unlocking_hex = "5a5b", .locking_hex = gte_not_lock, .expected = .{ .success = true } },
        .{ .row = 261, .name = "greaterthanorequal not over negative and positive becomes true", .unlocking_hex = neg11_11, .locking_hex = gte_not_lock, .expected = .{ .success = true } },
        .{ .row = 262, .name = "greaterthanorequal not over negative ascending becomes true", .unlocking_hex = neg11_neg10, .locking_hex = gte_not_lock, .expected = .{ .success = true } },
        .{ .row = 263, .name = "min of one and zero is zero", .unlocking_hex = "5100", .locking_hex = min_zero_eq_lock, .expected = .{ .success = true } },
