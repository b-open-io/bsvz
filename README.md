...
- the script interpreter now combines 691 unique exact Go row references across the split dedicated lanes with a 1,099-row filtered bulk-corpus lane and two conservative exact reference lanes for sigcheck and multisig mixed policy/result-shape rows (30 and 25 rows respectively); local corpus coverage remains comfortably past the earlier 75% milestone, but the full corpus is still not green
