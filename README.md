...
- the script interpreter is materially closer to Go parity, with the direct Go-row harness now split across dedicated control-flow, seam, bitwise, numeric, stack-shape, parser, multisig, sigcheck, reserved/NOP, and broader mixed-vector lanes; the full Go reference corpus is still not imported
...
| Go parity vectors | broad but incomplete | direct Go-row coverage now spans dedicated control-flow, seam, bitwise, numeric, stack-shape, parser, multisig, sigcheck, reserved/NOP, and broader mixed-vector lanes, but not the full Go corpus |
